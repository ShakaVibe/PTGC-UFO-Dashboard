...
DS_VOL_DRIFT=1 node run.js "#/ptgc" 1400 950 out/a52 "wait=6000;evalfile=probes/stale-tab.js;evalfile=probes/clock-shift.js;click=button[aria-label^='Refresh'];wait=4500;evalfile=probes/stale-tab.js"   # a52: each DexScreener answer reports a bigger 24 h volume; the clock shift steps past the 5-minute cache bucket so the refresh really re-fetches. headerVolume24h and valueGenFromVol must still MATCH afterwards (84,000 -> 378,000 on both, not just the header)
node -e 'const fs=require("fs"),j=JSON.parse(fs.readFileSync("data/value-generated.json"));j.meta.generatedAt=new Date(Date.now()-95*60000).toISOString();fs.writeFileSync("data/value-generated.json",JSON.stringify(j))' && HTML=index.html node run.js "#/ufo" 1400 950 out/a52lbl "wait=9000;evalfile=probes/stale-tab.js;eval=window.__shiftMin=180;evalfile=probes/clock-shift.js;click=button[aria-label^='Refresh'];wait=4500;evalfile=probes/stale-tab.js"   # a52: the "as of" labels must go 1.6h -> 4.6h, not stay frozen at load-time age (restore the file afterwards)
...
GT_CANDLES=1 node run.js "#/" 1440 960 out/home "wait=4000;shot=top"   # cosmic Home (2026-09-25): GT_CANDLES=1 serves synthetic GeckoTerminal 15-minute candles so both card charts draw; without it GeckoTerminal 404s and each card must show its five-point fallback curve
node run.js "?home=classic#/" 1440 900 out/classic   # the classic Home (the backup design) — must render exactly as before 2026-09-25
...
node run.js "#/ptgc" 1440 1000 out/vg "wait=5000;click=button:has-text('Socials'):visible;wait=1200;click=button:has-text('Value Generated'):visible;wait=2500;shot=wide;click=button[aria-label='Tall card, 4:5 for phones'];wait=1500;shot=tall"   # Value Generated share card (2026-09-25); "Token Allocation" the same way; `>> nth=1` on the button selector for the UFO column
